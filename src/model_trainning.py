...
class ModelTraining: 
    ...
    def save_model(self, model):
        try:
            logger.info(f"Creating directories for model output path")
            dir_path = os.path.dirname(self.model_output_path) or "."
            if not os.path.exists(dir_path):
                os.makedirs(dir_path, exist_ok=True)
                logger.info(f"Created model output directory: {dir_path}")
            else:
                logger.info(f"Model output directory already exists: {dir_path}")
    ...
    def run(self):
        try: 
            # Whenever this method run is called our mlflow will also start a new run
            with mlflow.start_run():
                logger.info("Starting model training pipeline")

                logger.info("Starting our MLFLOW experimentation")

                logger.info("Logging the training and dataset to MLFLOW")
            ...
            logger.error(f"Error occurred in model training pipeline: {e}")
            raise CustomException(f"Failed to run model training pipeline: {e}")
